...
const checkboxClass = (item) => {
  return [
    ...
    item.isChecked
      ? item.isDisabled
        ? "tree-item-checkbox-acitve-disabled"
        : "tree-item-checkbox-acitve"
      : "",
...
<template>
  <li
    ...
    v-for="(item, index) in innerDataSource"
    :key="index"
  >
    <div
      class="tree-item"
      :class="{
        'tree-item-active': activeIndex == item.label + '-' + index,
        'tree-item-disabled': item.isDisabled,
      }"
    >
      <!-- 没有子项不显示箭头 -->
      <span
    ...
    <transition name="tree-children-fade">
      <ul v-show="item.children && item.children.length != 0 && item.isOpen">
        <c-tree-child
          ...
          @handleNodeClick="handleChangeClick"
          @handleCheckboxClick="handleChangeCheckboxClick"
